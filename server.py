#!/usr/bin/env python3
"""
VidSnap 2.0 â€” yt-dlp powered video downloader
...
# â”€â”€ Auto-install yt-dlp â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
try:
    import yt_dlp
except ImportError:
...
# â”€â”€ HTML (served to browser) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
HTML = '''<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1">
<title>VidSnap â€” Video Downloader</title>
...
<div class="page">
  ...
  <header>
    ...
    <p class="sub">// Powered by yt-dlp Â· HD + Audio Â· 1000+ Sites</p>
  </header>
  ...
  <div class="card" style="animation-delay:.05s">
    ...
    <div class="url-field">
      <input type="url" id="urlInput"
        ...
        oninput="onUrlInput()"/>
      <button class="paste-btn" onclick="pasteUrl()" title="Paste from clipboard">ðŸ“‹</button>
    </div>
    <div class="platforms">
  ...
  <div class="card" style="animation-delay:.1s">
    ...
    <div class="quality-grid" id="qualityGrid">
      <div class="q-btn" onclick="setQ(this,'144p')" data-q="144p">
        ...
        <span class="q-note">ðŸ”Š w/ audio</span>
      </div>
  ...
  <!-- Footer -->
  <footer>
    VidSnap &nbsp;Â·&nbsp; Free &nbsp;Â·&nbsp; No ads &nbsp;Â·&nbsp; No limits
  </footer>
</div>
...
function log(msg, cls) {
  ...
  const icons = {'l-ok':'âœ“','l-er':'âœ—','l-info':'â†’','l-warn':'âš '};
...
async function startDownload() {
  ...
  if (!url) { showErr('âš ï¸ Please enter a video URL first!'); return; }
...
async function pollJob(id) {
  try {
    ...
    if (d.title || d.thumb) {
      ...
      document.getElementById('vcardMeta').textContent = d.duration ? 'â± ' + d.duration : '';
    }
    ...
    if (d.status === 'done') {
      ...
      a.innerHTML = `
        <div class="dl-link-left">
          ...
          <span class="dl-link-m">Tap to save Â· ${d.size || ''}</span>
        </div>
        <span class="dl-link-ico">â¬‡</span>`;
      list.appendChild(a);
      ...
      showErr('âŒ ' + (d.error || 'Download failed. Try a different quality or URL.'));
    }
  } catch(e) {
...
# â”€â”€ Download worker â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def run_download(job_id, url, quality):
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            err = next((l for l in reversed(jobs[job_id]['logs']) if 'ERROR' in l), 'Download failed')
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*52}")
    print(f"  ðŸŽ¬  VidSnap 2.0 is running!")
    print(f"{'='*52}")
    print(f"\n  ðŸ’»  Local:   http://localhost:{PORT}")
    print(f"  ðŸ“±  Network: http://{ip}:{PORT}")
    print(f"\n  ffmpeg: {'âœ“ Ready (HD+audio works)' if ffmpeg_ok else 'âœ— Missing! Run: sudo apt install ffmpeg'}")
    print(f"  yt-dlp: âœ“ Ready")
    print(f"  Downloads â†’ {DOWNLOAD_DIR}")
